 ...
 */
let nameInCaps = "Aisen"
let name = "aisen"

if nameInCaps.caseInsensitiveCompare(name) == .orderedSame {
...
if textToSearchThrough.localizedCaseInsensitiveContains(textToSearchFor) {
    print("I found it!")
}
//:  Print to the console the number of characters in your name by using the `count` property on `name`.
print("\(nameInCaps) contains \(name.count) letters")
